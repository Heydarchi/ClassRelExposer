...
class TestClassAnalyzer(TestCase):
    ...
    def test_find_class_pattern_java(self):
        ...
        for pattern in classAnalyzer.pattern[FileTypeEnum.JAVA]:
            ...
            self.assertEqual(inputStr[match.start():match.end()], "public class TestClass {")
    ...
    def test_find_class_pattern_java_with_inheritance(self):
        ...
        for pattern in classAnalyzer.pattern[FileTypeEnum.JAVA]:
            match = classAnalyzer.find_class_pattern(pattern, inputStr)
            self.assertEqual(inputStr[match.start():match.end()], "public class TestClass extends AbstractTestClass2 implements SuperTestClass{")
    ...
    def test_find_class_pattern_java_with_inheritance_and_comments(self):
        ...
        for pattern in classAnalyzer.pattern[FileTypeEnum.JAVA]:
            match = classAnalyzer.find_class_pattern(pattern, inputStr)
            self.assertEqual(inputStr[match.start():match.end()], " This is a comment */ public class TestClass extends AbstractTestClass2 implements SuperTestClass{")
